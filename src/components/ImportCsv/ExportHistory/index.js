...
import Collapsible from '../ImportHistory/Collapsible/Collapsible';
...
const ExportHistory = (props) => {
    ...
    const fetchData = async () => {
      try {
        const res = await api.get({
          resource: `enterprise-v3.0/exportHistory`,
        });
  ...
  const selectStartDate = (date) => {
    console.log('start', moment(date).valueOf());
    setStartDate(date);
    setFilter(true);
  };

  const selectEndDate = (date) => {
    console.log('end', moment(date).valueOf());
    setEndDate(date);
    setFilter(true);
  };
            ...
            <div className={localCss.listItem} key={i.uuid}>
              <div className={localCss.name}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <span className={localCss.span}>{i.fileName}</span>
                  <span>
                    {_l`By`} <strong>{i.username}</strong>
                  </span>
                </div>
              </div>
              <div className={localCss.date}>{moment(i.createdDate).format('DD MMM YYYY')}</div>
            </div>
          );
        })}
      </div>
