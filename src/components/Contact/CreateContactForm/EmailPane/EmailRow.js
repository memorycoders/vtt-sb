...
import CommunicationTypeDropdown from '../CommunicationTypeDropdown';
import { removeEmail, makeEmailMain, updateEmail } from '../../contact.actions';
import './styles.less';
...
const EmailRow = ({ email, removeEmail, formKey, makeEmailMain, updateEmail, errors }) => {
  ...
  let _dropdownId = `CreateContactCommunicationType-${email.uuid}`
  return (
    <Form className="email-row position-unset" key={email.uuid}>
      <Form.Group className="account-fields">
        ...
        <div className="email-field-type">
          <CommunicationTypeDropdown className={cx('position-clear', 'type-dropdown')} id={_dropdownId} onClick={() => calculatingPositionMenuDropdown(_dropdownId)} value={email.type} onChange={handleTypeChange} />
        </div>
