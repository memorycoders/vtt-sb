...
const LaneHelper = {
  ...
  moveCardAcrossLanes: (state, {
    ...
    cardToMove = fromLane.cards.find(card => card.id === cardId);
    if (!cardToMove){
      return {
        lanes: state.lanes
      }
    ...
    const interimLanes = state.lanes.map(lane => {
      if (lane.id === fromLaneId) {
        ...
        return (0, _immutabilityHelper.default)({
          ...lane,
          count: fromLaneId !== toLaneId ? Number(lane.count) - 1 : lane.count,
          grossValue: fromLaneId !== toLaneId ? Number(lane.grossValue) - (cardToMove ? Number(cardToMove.grossValue) : 0) : lane.grossValue,
        }, {
          cards: {
            $set: newCards
          }
        ...
        return {
          ...lane,
          count: fromLaneId !== toLaneId ? Number(lane.count) + 1 : lane.count,
          grossValue: fromLaneId !== toLaneId ? Number(lane.grossValue) + Number(cardToMove.grossValue) : lane.grossValue,
        }
      } else {
        return lane;
      }
