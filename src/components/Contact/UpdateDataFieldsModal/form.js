...
import SizeDropdown from '../../Organisation/CreateAccountForm/SizeDropdown';
...
class DataFieldsForm extends React.Component {
  ...
  handleRelationChange = (event, { value }) => {
    this.setState({ relationId: value }, () => {
      this.props.onChange(this.state);
    });
  };
  ...
  render() {
    const { street, zipCode, city, region, country, title, typeId, industryId, relationId, relationship } = this.state;
    ...
    return (
      <div className="unqualified-add-form">
        <Form>
          ...
          <Form.Group className="unqualified-fields">
            ...
            <div style={{ position: 'relative' }} className="dropdown-wrapper">
              <Input
                ...
                className="unqualified-area"
                value={region}
                onChange={(evt) => this.onChange('region', evt.target.value)}
              />
            </div>
          </Form.Group>
          <Form.Group className="unqualified-fields">
          ...
          <Form.Group className="unqualified-fields">
            <div className="unqualified-label">{_l`Position`}</div>
            <div style={{ position: 'relative' }} className="dropdown-wrapper">
              <RelationDropdown fluid value={relationId} onChange={this.handleRelationChange} />
            </div>
          </Form.Group>

          <Form.Group className="unqualified-fields">
            <div className="unqualified-label">{_l`Relationship`}</div>
            <div style={{ position: 'relative' }} className="dropdown-wrapper">
              <RelationshipDropdown fluid value={relationship} onChange={this.handleRelationShipChange} />
            </div>
          </Form.Group>

          <CustomFieldPane objectType={ObjectTypes.Contact} />
        </Form>
      </div>
    );
  }
