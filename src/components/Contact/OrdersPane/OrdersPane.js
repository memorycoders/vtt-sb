...
const OrdersPane = (props) => {
  const {data, dataInList, objectType, product, setTagManual, setProduct, loadMoreRows, orderBy, handleOrderBy, route, isFetching, requestFetchOrder} = props;
  ...
  const { orders, orderRows = [], totalOrderRow = 0 } = objectMerge; //oders được lấy ở địa chỉ state.entities.organisation.uuid 

  // hiển thị khi không có order
  ...
  const rightMenuProp = <RightMenu objectType={objectType} data={data} setProduct={setProduct} setTagManual={setTagManual} product={product} />

  return (
    <Collapsible right={rightMenuProp} count={orders ? orders.length : ''} open={true} width={308} title={product ? _l`Products` : _l`Orders`}
      isOrder={product ? false : true} account={data} requestFetchOrder={requestFetchOrder} >
      {
      isFetching ? (
        <div className={isFetching && `isFetching`}>
          <Loader active={isFetching}>Loading</Loader>
...
export default compose(
  connect((state, { data, objectType }) => {
    //data lấy từ state.entites.organisation[uuid].__DETAIL
    return {
      dataInList:
        objectType === ObjectTypes.Account
  ...
  lifecycle({
    componentWillReceiveProps(nextProps) {
      ...
      if (data.uuid !== nextProps.data.uuid) {
        let submitParams = {
          fromCreateDate: '1990-01-01',
          toCreateDate: moment(new Date()).format('YYYY-DD-MM'),
          status: '1',
          idNo: data?.taxCode,
          accountId: nextProps.data.uuid
        }
