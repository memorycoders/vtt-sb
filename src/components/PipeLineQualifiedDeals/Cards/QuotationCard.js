...
export const QuotationCard = (props) => {
  const { setActionForHighlight, qualifiedDeal, fetchQuotation, customerServiceList, totalRecord} = props;
  ...
  return (
    <>
                    ...
                    <TableRow key={e.uuid} style={{ cursor: 'pointer' }} onClick={() => showDetailQuotation(e)}>
                      ...
                      </TableCell>
                      {
                        !isFilterHistory && <TableCell textAlign="left" width={1}>
                          {(e.status === 'SENT' && !e.accepted) && <img src={star_blank} style={{ width: '15px', height: '25px' }}
                          // onClick={(event) => {openConfirmStatus(event, e)}} Nếu cần action thì bỏ comment dòng này ra
                           />}
                          {(e.status === 'SENT' && e.accepted) && <img src={star} style={{ width: '15px', height: '25px' }}
                          // onClick={(event) => {openConfirmStatus(event, e)}} Nếu cần action thì bỏ comment dòng này ra
                          />}
                        </TableCell>
                      }
      ...
      { showEdit && <EditQuotation visible={showEdit} uuid={quotationEditing} isCreate={isCreate}
                      onClose={closeEditQuotation} organisationUUID={qualifiedDeal?.uuid} /> }
      { quotationDelete && <DeleteQuotation visible={quotationDelete} uuid={uuid} onClose={closeDelete} /> }
      { showSendEmail && <SendQuotationByEmail uuid={uuid} visible={showSendEmail} onClose={closeSendEmail} /> }
