import React, { Component } from 'react';
import { connect } from 'react-redux';
import { Icon, Form, Input, Button } from 'semantic-ui-react';
import api from '../../lib/apiClient';
import css from '../../components/Form/FormInput.css';
import { setCaptchaId } from '../Auth/auth.actions';
import { getMessageError } from '../Auth/auth.selector';

export class CaptchaInputClass extends Component {
  constructor(props) {
    super(props);

    this.state = {
      captcha: {
        captchaId: 'hjqc7h7hs6nscedhbbiaj3qcat',
        captchaImg:
          '',
      },
      loading: false,
    };
  }
  componentDidMount() {
    this.getCaptchaImg();
  }
  componentWillReceiveProps(nextProps) {
    if (this.props.messageError !== nextProps.messageError && nextProps.messageError === 'INCORRECT_CAPTCHA') {
      this.reloadCaptcha();
    }
  }
  handleChange = (event, { value }) => {
    const {
      input: { onChange },
    } = this.props;
    onChange(value);
  };
  getCaptchaImg = async () => {
    this.setState({ loading: true });
    try {
      const res = await api.get({
        resource: `enterprise-v3.0/captchaImg`,
      });
      if (res) {
        // setLoading(false);
        // setCatcha(res);
        this.setState({
          captcha: res,
          loading: false,
        });
        this.props.setCaptchaId(res.captchaId);
      }
    } catch (e) {
      this.setState({
        loading: false,
      });
    }
  };
  reloadCaptcha = async () => {
    this.setState({
      loading: true,
    });
    try {
      const res = await api.get({
        resource: `enterprise-v3.0/reloadCaptchaImg/${this.state.captcha.captchaId}`,
      });
      if (res) {
        // setCatcha(res);
        // setLoading(false);
        this.setState({
          captcha: res,
          loading: false,
        });
        this.props.setCaptchaId(res.captchaId);
      }
    } catch (error) {
      console.log(error);
      this.setState({
        loading: false,
      });
    }
  };
  render() {
    const { meta, input, label, ...other } = this.props;
    const { value } = input;
    const { error, touched } = meta;
    const hasError = touched && !!error;
    return (
      <Form.Field error={hasError}>
        <div style={{ display: 'flex', justifyContents: 'space-between', alignItems: 'center' }}>
          <Input
            value={value ? value : ''}
            onBlur={input.onBlur}
            onFocus={input.onFocus}
            onChange={this.handleChange}
            {...other}
          />
          <img
            src={`data:image/png;base64, ${this.state.captcha?.captchaImg}`}
            height={37}
            style={{ minWidth: '40%', width: '40%', marginLeft: '3px', marginRight: '5px' }}
            alt="captchaImg"
          />
          {/* <Button type="button" icon={} /> */}
          <Icon loading={this.state.loading} name="refresh" size="large" onClick={this.reloadCaptcha} />
        </div>
        {hasError && <div className={css.error}>{error}</div>}
      </Form.Field>
    );
  }
}

const mapStateToProps = (state) => ({
  messageError: getMessageError(state),
});

const mapDispatchToProps = {
  setCaptchaId,
};

export default connect(mapStateToProps, mapDispatchToProps)(CaptchaInputClass);
