...
import { changeOnMultiMenu } from '../../components/PipeLineQualifiedDeals/qualifiedDeal.actions';
...
addTranslations({
  'en-US': {
    ...
    'Delete qualified deals': 'Delete qualified deals',
    Assign: 'Assign',
  ...
  return (
    <MoreMenu id="mutil_action" className={className} color={CssNames.Task}>
      ...
      <Menu.Item icon onClick={() => openDeleteTasksModal()}>
        <div className={css.actionIcon}>
          {overviewType == OverviewTypes.Pipeline.Order ? _l`Delete orders` : _l`Delete deals`}
