...
import CreateAccountForm from 'components/CallListAccount/CreateCallListAccountForm/CreateCallListAccountForm';
...
addTranslations({
  'en-US': {
    Cancel: 'Cancel',
    Save: 'Save',
    'Create account call list': 'Create account call list',
  },
});
  ...
  return (
    <Transition unmountOnHide visible={visible} animation="fade down" duration={UIDefaults.AnimationDuration}>
      <Modal open onClose={hideEditForm} size="small" closeIcon centered={false}>
        <Modal.Header className={css[Colors.Account]}>{_l`Add company call list`}</Modal.Header>
