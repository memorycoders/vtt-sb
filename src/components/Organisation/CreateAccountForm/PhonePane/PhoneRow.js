...
import { removePhone, makePhoneMain, updatePhone } from '../../organisation.actions';
import './styles.less';
...
const PhoneRow = ({ phone, removePhone, formKey, makePhoneMain, updatePhone }) => {
    ...
    <Form className="email-row position-unset" key={phone.uuid}>
      <Form.Group className="account-fields">
        ...
        <div className="email-field-type">
          <CommunicationTypeDropdown className={cx("position-clear", 'type-dropdown')} id={_dropdownId} onClick={() => calculatingPositionMenuDropdown(_dropdownId)} value={phone.type} onChange={handleTypeChange} />
        </div>
 ...
 {/* <div className="email-delete"> */}
          ...
          <div className={css.type}>
            <Button onClick={() => removePhone(formKey, phone.uuid)} className={css.deleteButton} icon="delete" size="mini" />
          </div>
            <div />
          {/* </div> */}
        {/* </div> */}
